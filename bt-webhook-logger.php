...
function btwl_handle_clear_logs() {
	// 检查是否是清空日志的 POST 请求，并验证用户权限
	if (isset($_POST['btwl_clear_logs']) && current_user_can('manage_options')) {
		check_admin_referer('btwl_clear_logs_nonce'); // 验证 Nonce，防止 CSRF 攻击
		
		global $wpdb;
		$table = $wpdb->prefix . 'btwl_logs';
		$wpdb->query("TRUNCATE TABLE $table"); // 清空数据表，效率更高

		// 添加管理通知，使用 WordPress 推荐的 add_settings_error
		add_settings_error(
			'bt-webhook-logger-messages', // 错误设置 ID
			'log-cleared',                // 错误代码
			'WebHook 日志已清空！',       // 消息内容
			'success'                     // 消息类型
		);
...
function btwl_handle_settings_save() {
	// 检查是否是保存设置的 POST 请求，并验证用户权限
	if (isset($_POST['btwl_save_settings']) && current_user_can('manage_options')) {
		check_admin_referer('btwl_settings_nonce'); // 验证 Nonce

		// 清理用户输入的 access_key，防止恶意代码
		$new_access_key = sanitize_text_field($_POST['btwl_access_key']);
		update_option('btwl_access_key', $new_access_key); // 保存到 WordPress 选项
...
/* ---------- 6. 注册管理通知显示函数 ---------- */
// 确保在 admin_notices 钩子上显示通过 add_settings_error 添加的通知
