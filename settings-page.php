...
?>
<div class="wrap">
	<h1><?php esc_html_e('aaPanel WebHook 设置', 'webhook-logger-for-aapanel'); ?></h1>
	...
	<form method="post">
		...
		<table class="form-table">
			<tr class="btwl-settings-section">
				...
				<td>
					...
					<p class="description"><?php esc_html_e('设置一个 Access Key 来保护你的 WebHook。留空表示不需要 Access Key（不推荐）。', 'webhook-logger-for-aapanel'); ?></p>
					<p class="description"><?php printf(/* translators: %s is the WebHook URL */ esc_html__('你的 WebHook 地址: %s', 'webhook-logger-for-aapanel'), esc_url($webhook_url_example)); ?></p>
					<?php if (empty($current_access_key)) : ?>
						<p class="description" style="color: red;"><?php esc_html_e('当前未设置 Access Key，WebHook 地址对所有请求开放，存在安全风险。', 'webhook-logger-for-aapanel'); ?></p>
					<?php endif; ?>
				</td>
			</tr>
			<tr class="btwl-settings-section">
				...
				<td>
					<label for="btwl_enable_email">
						...
						<?php esc_html_e('启用 WebHook 邮件通知', 'webhook-logger-for-aapanel'); ?>
					</label>
			...
			<tr class="btwl-settings-section">
				...
				<td>
					...
					<p class="description"><?php esc_html_e('接收 WebHook 通知邮件的邮箱地址。请确保您的 WordPress 已正确配置邮件发送服务。', 'webhook-logger-for-aapanel'); ?></p>
					<?php if (!empty($current_enable_email) && !is_email($current_target_email)) : ?>
						<p class="description" style="color: red;"><?php esc_html_e('邮件通知已启用，但目标邮箱地址无效，请检查。', 'webhook-logger-for-aapanel'); ?></p>
					<?php endif; ?>
				</td>
			</tr>
		</table>
		<p class="submit">
			...
			<button type="button" class="button button-secondary" onclick="document.getElementById('btwl_access_key').value = generateRandomKey();"><?php esc_html_e('生成随机密钥', 'webhook-logger-for-aapanel'); ?></button>
		</p>
	</form>
</div>
